...
- **Your LangSmith API key and MailHog/digest email settings are in your local `.env`** (gitignored, never committed): `LANGCHAIN_TRACING_V2=true` (traces land in the `smartreco` project on your dashboard — the code's own default, no override needed anymore), `SMTP_HOST=mailhog`/`SMTP_PORT=1025`/`DIGEST_FROM_EMAIL=digest@smartreco.dev` (digest emails land in MailHog's web UI at `localhost:8025` when the stack is running).
...
### 2026-08-06 — Fix: LangSmith `LANGSMITH_TRACING` (bare, no `_V2`) + project renamed back to `smartreco` (PR #21)
- You pointed out your LangSmith dashboard's current quickstart snippet shows `LANGSMITH_TRACING` (no `_V2` suffix) - `configure_langsmith()` only set the `_V2` forms. Confirmed via the installed SDK's source that `_V2` was already checked first regardless (not a functional bug), but `configure_langsmith()` now sets both so the env vars actually match what the official docs show.
- Renamed the local verification project back to the code's own default (`smartreco`) by removing the `LANGCHAIN_PROJECT` override from `.env` - live-verified via the LangSmith API that a fresh `smartreco` project was created with a successful `LangGraph` run in it.

### 2026-08-06 — UI redesign (every screen) + interactive, personalized digest email + `next`-redirect login (PR #22)
- **Full second UI pass**, at your request ("the UI as of now is really bad... make the UI designs more perfect and attractive"), made without committing until you'd reviewed it live in your browser across several rounds. Richer visual system throughout `style.css` (gradients, layered shadows, hover-lift, tonal product covers with a pattern overlay, decorative glow on hero/empty states), nav icons + avatar, a real home-page hero + feature grid, real admin dashboard stat cards (DB-backed, not fabricated), and a full-screen split-panel redesign of login/register (gradient brand panel with a decorative product-card mockup, icon-accented inputs, password show/hide toggle).
- **You caught a real CSS bug in the auth pages**: `width: 100vw` (used to break the panel out of `main`'s centered max-width) counts the scrollbar's own width in Chrome, causing a horizontal scrollbar and a "zoomed in" look; combined with `main`'s own `min-height` + the footer's height, the page also scrolled vertically when it shouldn't have. Fixed properly rather than patching around it - `main:has(.auth-shell)` neutralizes `main`'s max-width/padding directly and `body:has(.auth-shell) .footer` hides the footer, so `.auth-shell` can just be `height: 100vh` with no viewport-unit tricks at all.
- **Digest email now sends `multipart/alternative`** (styled HTML + plain-text fallback) instead of plain text only - built via a decoded-MIME preview artifact so you could see exactly what MailHog actually stored, not a mockup. Added a "View in Dashboard" button and made every course row a real clickable link to its detail page (new `PUBLIC_BASE_URL` setting - an email is opened outside any browser session tied to the app's host, so links there must be absolute). Per your follow-up feedback, the narrative is now framed with an explicit "based on your recent activity, chosen specifically for you" line instead of dropping straight into the raw LLM sentence.
- Those new email links needed a `next`-redirect for login/register (`safe_next_path()` guards against open redirects) - clicking the dashboard button while signed out now goes through login and lands back on `/recommendations`, not the generic `/catalog`.
- 108/108 tests passing, `ruff` clean. Verified live throughout: real digest emails sent via MailHog and inspected through its own API; full end-to-end login-redirect flow tested with a real HTTP client (a `curl`-argv path-mangling false alarm from Git Bash was ruled out via a direct Python HTTP check); every page smoke-tested after each change. Squash-merged to `main`, live stack rebuilt.
