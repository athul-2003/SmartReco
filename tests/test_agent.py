...
def test_generate_recommendation_returns_grounded_product_ids(
    ...
    fake_client = type(
        "FakeClient",
        (),
        {"chat": lambda self, messages: "Because you like Dev courses."},
    )()
    monkeypatch.setattr(nodes, "get_llm_client", lambda: fake_client)

    narrative, product_ids = nodes.generate_recommendation(session, user)
    assert narrative == "Because you like Dev courses."
    assert product_ids == [1, 2]


def test_generate_recommendation_handles_no_candidates(session: Session, monkeypatch):
    user = _make_user(session)
    monkeypatch.setattr(nodes, "embed_texts", lambda texts: [[0.1, 0.2, 0.3]])
    monkeypatch.setattr(nodes.vector_store, "search", lambda vector, top_k=5: [])
