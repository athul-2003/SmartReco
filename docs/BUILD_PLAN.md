...
**Tasks**
- [x] **LangGraph (FR-4.5):** refactor `agent/nodes.py` into an explicit `agent/graph.py` graph — analyze → decide-to-retrieve → evaluate retrieval quality → refine-if-weak → generate
- [x] **APScheduler (FR-6):** `scheduler.py` — in-process daily job that runs the same agent pipeline per active user and emails the digest
- [x] **Email delivery:** `services/email.py` sending via SMTP (`smtplib`), configured through `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`DIGEST_FROM_EMAIL` env vars
- [x] **LangSmith (FR-7):** tracing enabled via env vars (`LANGCHAIN_TRACING_V2`, `LANGCHAIN_API_KEY`) around the agent graph — no code-path branching needed, it's instrumentation-only
- [x] **Metadata filtering:** extend Qdrant retrieval to optionally filter by `category`/`price` payload fields

**Decision:** email digest is sent via **plain SMTP**, not a paid provider (e.g. AWS SES) — zero added cost/cloud dependency, fits the "no extra broker/service" philosophy already used for APScheduler; SES would need a real AWS account, IAM credentials, and sender/domain verification for a feature that's a bonus, judged mainly on "did it run and produce correct output." If `SMTP_*` env vars are unset (e.g. during CI or a reviewer's clone-and-run), the digest job **logs the rendered email to console/file instead of sending** — so the scheduled job never crashes a demo for lack of mail credentials, and the feature is still visibly exercised. **Added:** a `mailhog` service to `docker-compose.yml`/`.override.yml` purely for local dev testing — a throwaway SMTP catcher with a local web UI (`localhost:8025`), so the real "sent" code path (not just the logged fallback) can be demonstrated without needing your actual email credentials or spamming a real inbox. `SMTP_HOST=mailhog`/`SMTP_PORT=1025` in the dev override; production still points at whatever real `SMTP_*` you configure.

**You provide (optional):** real SMTP credentials (e.g. a Gmail App Password or any SMTP relay) if you want actual emails during your own testing; not required for the app to run or demo — MailHog covers local verification of the send path, and log-fallback covers the no-credentials case.

**Sub-task — LangGraph refactor:**
- `app/agent/graph.py` — a `langgraph.graph.StateGraph` with 3 nodes (`analyze`, `retrieve`, `generate`) and one conditional edge out of `retrieve`: if the top Qdrant match's cosine score is below `WEAK_SCORE_THRESHOLD` (0.35) and fewer than `MAX_RETRIEVAL_ATTEMPTS` (2) have run, loop back into `retrieve` with a 3x wider `top_k` (`REFINED_TOP_K`) before generating; otherwise proceed straight to `generate`. Reuses `build_profile`/`retrieve_candidates`/`generate_narrative` from `agent/nodes.py` as node bodies rather than duplicating their logic.
- **Scoped to `POST /refresh` only** (`routers/recommendations.py` now imports `run_recommendation_graph` aliased as `generate_recommendation`, so the router's own code didn't need to change). The first-generation and auto-regenerate-on-threshold paths (`GET /recommendations`) deliberately **keep** using `nodes.prepare_candidates` + `generate_narrative_stream` instead of the graph — that split exists specifically so the narrative can stream incrementally (Phase 4 follow-up #3 / Phase 5's threshold trigger); a single blocking `graph.invoke()` call would undo that UX. `nodes.generate_recommendation()` (the old straight-chain manual-refresh function) was removed rather than left as unused dead code now that the graph replaces it.
- Chose an in-process, unpersisted graph (no LangGraph checkpointer) — nothing in this pipeline needs to pause/resume across requests; the state dict (profile, candidates, retrieval_attempts, narrative, product_ids) only needs to live for the duration of one `invoke()` call.

**Definition of done (LangGraph):** ✅ Verified. 64/64 tests pass, including new `tests/test_graph.py` (grounded-product-ids happy path; empty-candidates fallback; weak-score triggers exactly one wider refine attempt; a permanently-weak result still terminates at `MAX_RETRIEVAL_ATTEMPTS` instead of looping forever). `ruff` clean. **Live, against the real rebuilt stack:** registered a user, generated real Business-category browsing events, hit `POST /refresh` — completed in ~9.4s (real Mesh embed + Qdrant + Mesh chat through the graph), produced a narrative correctly grounded in the actual behavioral signal ("you've shown interest in Business"), and all 5 recommended product IDs confirmed as real rows in the container's SQLite DB. No errors in container logs.

**Sub-task — Metadata filtering:**
- `vector_store._build_filter(category, max_price)` builds a Qdrant `Filter` (`FieldCondition`/`MatchValue` for category, `FieldCondition`/`Range(lte=...)` for price) against the `category`/`price` payload fields already stored on every point at seed/upsert time - no new data needed. `search()` passes it as `query_filter`; `None`/`None` means no filter, unchanged behavior.
- Threaded through the whole call chain: `vector_store.search` → `nodes.retrieve_candidates` → `graph.GraphState`/`_retrieve` (both the initial and refined retrieval attempts respect the same filter) → `graph.run_recommendation_graph`. Exposed as optional `category`/`max_price` query params on `POST /refresh`, so it's a real, callable capability end-to-end (not just internal plumbing) - omitted, refresh behaves exactly as before.
- **Definition of done (metadata filtering):** ✅ Verified. 73/73 tests pass, including new `tests/test_vector_store.py` (`_build_filter` construction for category-only/price-only/both/neither; `search()` passes the built filter through to `query_points`, or `None` when unfiltered) plus pass-through tests in `test_agent.py`/`test_graph.py`/`test_recommendations.py`. `ruff` clean. **Live, against the real Qdrant collection (1,500 seeded products):** `category=Business` narrows 1500 → 300; adding `price<=2000` narrows further to 74; called through the real `POST /refresh?category=Business&max_price=2000` HTTP endpoint and confirmed all 15 returned recommendations are genuinely Business-category and ≤₹2000 by querying the container's real SQLite DB directly - zero results violating the filter.
- Incidental observation while live-testing: filtering to a narrower corpus can push the top-candidate score below `WEAK_SCORE_THRESHOLD` more often (fewer/weaker semantic matches within the filtered subset), which correctly engaged the LangGraph refine step (retry at 3x `top_k`) built in the prior sub-task - the two Phase 6 features compose correctly together, not just individually.

**Sub-task — LangSmith tracing:**
- `app/observability.py::configure_langsmith(settings)` — called once at `main.py` startup. If `LANGCHAIN_TRACING_V2` + `LANGCHAIN_API_KEY` are set, propagates them (plus a new `LANGCHAIN_PROJECT` setting, defaulting to `"smartreco"`) into `os.environ`, since pydantic-settings parses `.env` into our own `Settings` object without touching `os.environ` - and the LangSmith SDK's own auto-instrumentation (which wraps every LangGraph node automatically, no explicit tracer/callback wiring in `agent/graph.py`) reads `os.environ` directly.
- **Sets both `LANGSMITH_*` and `LANGCHAIN_*` env var names**, not just the `LANGCHAIN_*` names the SRS/task list literally names. Reading the installed `langsmith` SDK's source directly (`langsmith/utils.py::get_env_var(namespaces=("LANGSMITH", "LANGCHAIN"))`) confirmed `LANGSMITH_*` takes precedence when both are present - the modern SDK checks it first, `LANGCHAIN_*` is a fallback for older versions/back-compat. Setting both is correct either way and matches what a LangSmith dashboard's own quickstart snippet shows (`LANGSMITH_PROJECT=...`), which is what surfaced this during your own live testing.
- **Found and fixed a real test-isolation bug while live-verifying:** `app.main` (imported by every test via `conftest.py`) calls `configure_langsmith()` at module-import time. With real tracing enabled in a developer's local `.env` (needed for the live verification below), running `uv run pytest` was silently sending every automated test's real `LangGraph` invocations as live traces to LangSmith - a real violation of "never hit real external services in automated tests" (previously only enforced for Mesh/Qdrant/SMTP). Confirmed via the LangSmith API: found 4 traces clustered within ~1.3s matching `test_graph.py`'s 4 graph-invoking tests. Fixed with an `autouse` fixture in `tests/conftest.py` that clears the `LANGSMITH_*`/`LANGCHAIN_*` tracing env vars before every test - `tracing_is_enabled()` re-checks them at call time (not just at import), so this reliably blocks it regardless of the developer's own `.env`. Reverified: re-ran the full suite with a real LangSmith project configured, queried the API again, confirmed zero new traces landed.
- **Definition of done (LangSmith):** ✅ Verified. 77/77 tests pass (new `tests/test_observability.py`: no-op when tracing disabled/key missing, both namespaces set correctly when configured, default project name used when unset - isolated from the developer's own `.env` via `_env_file=None`). `ruff` clean. **Live, with your real LangSmith API key** (`.env`, gitignored, never committed): rebuilt/restarted the container, ran a real `POST /refresh` through the LangGraph pipeline, then queried the LangSmith REST API directly (`/sessions` + `/runs/query`) rather than just trusting the dashboard visually - confirmed a root `LangGraph` run with `status: "success"` landed in your named project (`pr-downright-collard-46`, not the code's own `"smartreco"` default), with the full node structure nested correctly underneath it (`analyze` → `retrieve` → `_evaluate_retrieval` → `generate`, matching the graph topology from the LangGraph sub-task above exactly).

**Sub-task — APScheduler + email digest:**
- `app/services/email.py::send_email()` — plain `smtplib`, `EmailMessage`. If `settings.smtp_host` is unset, logs the fully rendered email instead of sending (the SRS's own fallback decision, above). If configured, only calls `starttls()`/`login()` when `smtp_user`/`smtp_password` are actually set - a local dev catcher like MailHog has no auth/TLS, so unconditionally calling either would break the local-testing path this same sub-task adds.
- `app/services/digest.py::run_daily_digest(session)` — "active user" = has at least one tracked `Event` (same premise as the cold-start empty state). For each, runs the same `run_recommendation_graph()` LangGraph pipeline the manual refresh button uses (reuse, not a parallel pipeline), skips (no email) users with no grounded candidates rather than sending an empty digest, and returns a count - kept separate from `scheduler.py` specifically so tests (and this section's live verification) can call it directly against a real `Session` without needing the actual APScheduler timer loop to fire, per this phase's testing bar.
- **Deliberately does not persist a `Recommendation` row.** The digest email is a side-channel nudge, not a state change to what `GET /recommendations` shows on next visit - persisting one would silently interact with Phase 5's caching/threshold logic in a way nothing asked for (the user didn't request a refresh, so their cached view shouldn't change from a scheduled email they may not even open).
- `app/scheduler.py` — `apscheduler.schedulers.background.BackgroundScheduler` (a dedicated thread, not asyncio) since the whole pipeline underneath is synchronous, matching the project's existing sync-everywhere decision (Phase 1). One cron job (`daily_digest`, 08:00 server time), started/stopped from `main.py`'s `lifespan`.
- **MailHog added to `docker-compose.override.yml`** (dev-only, per your call on email delivery at the start of this phase - you picked plain SMTP over AWS SES, plus MailHog for local verification of the real "sent" path): `mailhog/mailhog:v1.0.1`, web UI at `localhost:8025`, SMTP catcher at `1025`. Set `SMTP_HOST=mailhog`/`SMTP_PORT=1025` in `.env` (left `SMTP_USER`/`PASSWORD` blank - MailHog has no auth) to exercise it. Excluded from `make prod-*` automatically, same as the rest of `docker-compose.override.yml`.
- **Definition of done (APScheduler + email):** ✅ Verified. 87/87 tests pass (new `tests/test_email.py`: log-fallback, unauthenticated send for a MailHog-style target, authenticated send with `starttls`/`login` for a real relay; new `tests/test_digest.py`: active-user detection, skip-if-no-candidates, email body rendering including the free-vs-priced case; new `tests/test_scheduler.py`: job registration, start idempotency, stop-without-start no-op). `ruff` clean. **Live, against the real running stack with a real MailHog container** (no mocks): registered two fresh real users ("alice"/"bob") with real tracked events, called `run_daily_digest()` directly inside the container against the real DB/Mesh/Qdrant/LangGraph pipeline - sent 20 real digest emails (every active user accumulated across this session's live testing, not just the 2 new ones). Queried MailHog's own REST API (`/api/v2/messages`) directly, not just the web UI, and confirmed all 20 landed with the correct `To`/`Subject`; inspected one full body and confirmed it was a real, coherent, behaviorally-grounded narrative with real product titles/prices (zero fabricated data) - the same quality bar as the manual refresh path, because it's the literal same pipeline.

**Definition of done (full Phase 6):** ✅ **Phase 6 complete.** LangGraph run visible in LangSmith with all steps traced (verified via the LangSmith API); a manually-triggered digest run produces a correctly rendered email (verified sent via MailHog, and unit-verified logged when `SMTP_*` is unset); category-filtered retrieval returns narrower results than unfiltered (verified against the real 1,500-product Qdrant collection).

**Phase 6 status: ✅ Complete** (all 5 bonus tasks: LangGraph, metadata filtering, LangSmith tracing, APScheduler, email delivery).
