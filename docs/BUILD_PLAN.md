...
**Definition of done (full Phase 6):** LangGraph run visible in LangSmith with all steps traced; a manually-triggered digest run produces a correctly rendered email (sent via MailHog locally, or logged when `SMTP_*` is unset); category-filtered retrieval returns narrower results than unfiltered.
